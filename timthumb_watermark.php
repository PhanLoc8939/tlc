...
function show_image ($mime_type, $image_resized, $quality, $cache_dir, $new_width) {
	...
	$d->reset();
	$sql_dongdau = "select  photo from #_photo where com='background' limit 0,1";
	$d->query($sql_dongdau);
	...
	//$watermark = imagecreatefromfile('./upload/hinhanh/wtm.png');
	$watermark = imagecreatefromfile('./upload/hinhanh/'.$dongdau['photo'].'');
	...
	//$watermark_pos_x = imagesx($image_resized) - imagesx($watermark) - 0;
    //$watermark_pos_y = imagesy($image_resized) - imagesy($watermark) - 0;
