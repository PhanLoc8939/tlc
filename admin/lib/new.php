...
$today            =    'Hôm nay : '; 
	...
	$query             =    "SELECT COUNT(id) AS total FROM counter"; 

    //$t = mysql_fetch_assoc(mysql_query($query)); 

	$t = mysql_fetch_assoc($d->query($query)); 
	...
	$query             =    "SELECT COUNT(*) AS dangonline FROM counter WHERE tm>'".(time()-600)."'"; 

    $dangonlinerc  = mysql_fetch_assoc($d->query($query)); 

    $online_visitors     =    $dangonlinerc['dangonline']; 
