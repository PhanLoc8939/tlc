...
function sessionhash(){
  ...
  $password_update = encrypt_password($token_temp,$password,$config['salt']);
...
function encrypt_password($token,$str,$salt){

	return sha1(md5($token.$str.$salt));
...

{
	  ...
	  $dime_x = ($size[0] - $water_width)/1;  

	  $dime_y = ($size[1] - $water_height)/1;
...
function paging_home($r, $url='', $curPg=1, $mxR=5, $mxP=5, $class_paging='')
    ...
    border-radius: 5px; height:27px;line-height:27px;border:1px solid #ddd ' >
			...
			#{
				...
				$paging .=" <a href='".$url."p=".($totalPages)."' style='display: inline-block; color: #fff; background:#fff;  font-weight: bold;width:38px;    -webkit-border-radius: 5px;
