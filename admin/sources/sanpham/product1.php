...
switch($act){	
	...
	case "man_quatang":
		...
		$template = "sanpham/product1/quatang/cats";

		break;

	case "add_quatang":		

		$template = "sanpham/product1/quatang/cat_add";
...
function get_quatangs(){
...

	{
		...
		redirect("index.php?com=product1&act=man_quatang".((@$_REQUEST['curPage']!="")?("&curPage=".$_REQUEST['curPage'].""):""));	
	...
	$sql = "select * from #_product_phu where com='quatang' order by stt asc,id desc";
	...
	$url="index.php?com=product1&act=man_quatang";
...
function get_quatang(){
	...
	transfer("Không nhận được dữ liệu", "index.php?com=product1&act=man_quatang".((@$_REQUEST['curPage']!="")?("&curPage=".$_REQUEST['curPage'].""):""));
	...
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=product1&act=man_quatang".((@$_REQUEST['curPage']!="")?("&curPage=".$_REQUEST['curPage'].""):""));
...
function save_quatang(){

	global $d;			

	

	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=product1&act=man_quatang".((@$_REQUEST['curPage']!="")?("&curPage=".$_REQUEST['curPage'].""):""));
	...
	if($id){	
		...
		if($d->update($data))
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product1&act=man_quatang".((@$_REQUEST['curPage']!="")?("&curPage=".$_REQUEST['curPage'].""):""));
		...
		if($d->insert($data))
			...
			redirect("index.php?com=product1&act=man_quatang");

		else

			transfer("Lưu dữ liệu bị lỗi", "index.php?com=product1&act=man_quatang".((@$_REQUEST['curPage']!="")?("&curPage=".$_REQUEST['curPage'].""):""));
...
function delete_quatang(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=product1&act=man_quatang".((@$_REQUEST['curPage']!="")?("&curPage=".$_REQUEST['curPage'].""):""));

	}else transfer("Không nhận được dữ liệu", "index.php?com=product1&act=man_quatang".((@$_REQUEST['curPage']!="")?("&curPage=".$_REQUEST['curPage'].""):""));
