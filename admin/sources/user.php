...
function login(){
	...
	global $d, $login_name,$config,$row,$username,$password;
	...
	if($d->num_rows() == 1){	
				...
				// echo encrypt_password($row['token'],$password,$config['salt']);
				// die('x');

			if($row['password'] == taomatkhau($password)){
			...
			else
			{
					...
					transfer("Mật khẩu không chính xác", "index.php?com=user&act=login");
	...
	}else
	{
			...
			transfer("Tên đăng nhập không chính xác", "index.php?com=user&act=login");
	}
