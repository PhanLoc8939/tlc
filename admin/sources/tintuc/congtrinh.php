...
function save(){
	...
	$data['id_cat2'] = intval($_POST['id_cat2']);	
		...
		foreach ($config["lang"] as $key => $value)  {
            ...
            $data['noidung_'.$value] = magic_quote($_POST['noidung_'.$value]);
              ...
              $data['tenchudautu'] = magic_quote($_POST['tenchudautu']);
              $data['quymo'] = magic_quote($_POST['quymo']);
              $data['tongdientich'] = magic_quote($_POST['tongdientich']);
              $data['diachi'] = magic_quote($_POST['diachi']);
               $data['loaihinh'] = magic_quote($_POST['loaihinh']);
              $data['gia'] = magic_quote($_POST['gia']);
              $data['vitri'] = magic_quote($_POST['vitri']);
               $data['hatang'] = magic_quote($_POST['hatang']);
               $data['thietke'] = magic_quote($_POST['thietke']);
               $data['matbang'] = magic_quote($_POST['matbang']);
               $data['thuvienanh'] = magic_quote($_POST['thuvienanh']);
               $data['tiendo'] = magic_quote($_POST['tiendo']);
               $data['chudautu'] = magic_quote($_POST['chudautu']);
               $data['thanhtoan'] = magic_quote($_POST['thanhtoan']);
               $data['dientichxaydung'] = magic_quote($_POST['dientichxaydung']);
                $data['matdoxaydung'] = magic_quote($_POST['matdoxaydung']);
               $data['bangiaonha'] = magic_quote($_POST['bangiaonha']);
                 $data['tinhtrang'] = magic_quote($_POST['tinhtrang']);
                 		$data['id_tinh_duan'] = magic_quote($_POST['id_tinh']);
		$data['id_quan_duan'] = magic_quote($_POST['id_quan']);
	...
	if($id){		
		if($photo = upload_image("file", 'jpg|png|gif|jpeg|JPEG|JPG|PNG|GIF',_upload_congtrinh,$file_name)){
			$data['photo'] = $photo;
			$data['thumb'] = create_thumb($data['photo'], 1366, 397, _upload_congtrinh,$file_name,2);
			$d->setTable('congtrinh');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
		...
		if($photo1 = upload_image("file1", 'jpg|png|gif|jpeg|JPEG|JPG|PNG|GIF',_upload_congtrinh,$file_name1)){
			$data['photo1'] = $photo1;
			$d->setTable('congtrinh');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_congtrinh.$row['photo1']);
			}
		}
		if($photo2 = upload_image("file2", 'jpg|png|gif|jpeg|JPEG|JPG|PNG|GIF',_upload_congtrinh,$file_name2)){
			$data['photo2'] = $photo2;
			$d->setTable('congtrinh');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_congtrinh.$row['photo2']);
			}
		}
		if($photo3 = upload_image("file3", 'jpg|png|gif|jpeg|JPEG|JPG|PNG|GIF',_upload_congtrinh,$file_name3)){
			$data['photo3'] = $photo3;
			$d->setTable('congtrinh');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_congtrinh.$row['photo3']);
			}
		}
		if($photo4 = upload_image("file4", 'jpg|png|gif|jpeg|JPEG|JPG|PNG|GIF',_upload_congtrinh,$file_name4)){
			$data['photo4'] = $photo4;
			$d->setTable('congtrinh');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_congtrinh.$row['photo4']);
			}
		}
...
function save_photo(){
	...
	if($id){
		...
		if($photo = upload_image("file", 'jpg|png|gif|jpeg|JPEG|JPG|PNG|GIF',_upload_congtrinh,$file_name)){
			$data['photo'] = $photo;
			// $data['thumb'] = create_thumb($data['photo'], 450, 327, _upload_congtrinh,$file_name,2);
