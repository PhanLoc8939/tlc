...
function save($c,$act,$width,$height,$truonghop){
	...
	$data['loai_cat'] = magic_quote($_POST['loai_cat']);



	$data['tenkhongdau_vi'] = q_bodautv($_POST['ten_vi']);
	foreach ($config["lang"] as $key => $value)  {

		$data['ten_'.$value] = magic_quote($_POST['ten_'.$value]);
		$data['chucvu_'.$value] = magic_quote($_POST['chucvu_'.$value]);
	...
	if($id){		
		...
		if($logo = upload_image("file2", 'jpg|png|gif|jpeg|JPEG|JPG|PNG|GIF|doc|DOC|docx|DOCX|pdf|PDF|xlsx|XLSX',_upload_tinnho,$file_name2)){
			$data['logo'] = $logo;
