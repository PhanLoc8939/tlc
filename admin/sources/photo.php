...
switch($act){
	...
	$template = "photo/video/items1";

	break;

	case "add1_video":

	$template = "photo/video/item1_add";
	...
	save('slideshow2','man2',256,192,2);
...
function save_banner($c,$act){
	...
	if($id){ // cap nhat
		...
		if($photo = upload_image("file", 'jpg|gif|png|jpeg|doc|docx||pdf|xlsx', _upload_sanpham,$file_name)){
			$data['photo'] = $photo;
		...
		if($photo1 = upload_image("file1", 'swf|png|gif|jpeg|jpg|SWF|PNG|GIF|JPEG|JPG', _upload_hinhanh,$file_name1)){
	...
	if($photo = upload_image("file", 'jpg|gif|png|jpeg|doc|docx||pdf|xlsx', _upload_hinhanh,$file_name)){
			$data['photo'] = $photo;
		}
...
function save_quangcao2ben($c,$act){
	...
	if($id){
		...
		if($photo1 = upload_image("file1", 'jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF', _upload_hinhanh,$file_name1)){
...
function get1_videos(){

	global $d, $items, $paging, $chuoi_noi_curpage;	

	thaydoi_hienthi('hienthi','table_photo','photo','man1_video',$chuoi_noi_curpage);			


	$sql = "select * from #_photo where com='video1' order by stt asc,id desc";
	...
	$url="index.php?com=photo&act=man1_video";
...
function get1_video(){
	...
	if(!$id)
		...
		transfer("Không nhận được dữ liệu", "index.php?com=photo&act=man1_video".$chuoi_noi_curpage);

	$sql = "select * from #_photo where id=$id and com='video1'";

	$d->query($sql);

	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=photo&act=man1_video".$chuoi_noi_curpage);
...
function save1_video(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=photo&act=man1_video".$chuoi_noi_curpage);
	...
	if($id){
		...
		if($download = upload_image("file", 'jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF|mp4|MP4|AVI|avi',_upload_hinhanh,$file_name)){
			$data['download'] = $download;
			
			$d->setTable('photo');
			$d->setWhere('id', $id);
			$d->setWhere('com', 'video1');
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_hinhanh.$row['download']);
		...
		if($d->update($data))
			...
			redirect("index.php?com=photo&act=man1_video".$chuoi_noi_curpage);

		else

			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=photo&act=man1_video".$chuoi_noi_curpage);
		...
		if($d->insert($data))
			...
			redirect("index.php?com=photo&act=man1_video");

		else

			transfer("Lưu dữ liệu bị lỗi", "index.php?com=photo&act=man1_video".$chuoi_noi_curpage);
...
function delete1_video(){
	...
	if(isset($_GET['id'])){
		...
		$sql = "delete from #_photo where id=$id and com='video1'";		
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=photo&act=man1_video".$chuoi_noi_curpage);

	}else transfer("Không nhận được dữ liệu", "index.php?com=photo&act=man1_video".$chuoi_noi_curpage);
...
function save_hotline(){
	...
	if($id){		
		...
		if($photo = upload_image("file", 'jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF|MP3|mp3', _upload_hinhanh,$file_name)){
		...
		if($photo1 = upload_image("file1", 'jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF|MP3|mp3', _upload_hinhanh,$file_name1)){
...
function gets($c,$act){
	...
	thaydoi_hienthi('hienthi','table_photo','photo',$act,$chuoi_noi_curpage);		
	thaydoi_hienthi('noibat','table_photo','photo',$act,$chuoi_noi_curpage);
	thaydoi_hienthi('hot','table_photo','photo',$act,$chuoi_noi_curpage);
...
function save($c,$act,$width,$height,$truonghop){
	...
	if($id){			
		...
		if($photo2 = upload_image("file2", 'jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF', _upload_hinhanh,$file_name2)){
		...
		if($photo3 = upload_image("file3", 'jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF', _upload_hinhanh,$file_name3)){
