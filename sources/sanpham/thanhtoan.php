<?php  if(!defined('_source')) die("Error");		
	...
	if ($coloi==FALSE) {
       ...
       $body.='</table>';
			...
			$sql = "INSERT INTO  table_donhang (idmember,madonhang,hoten,dienthoai,diachi,email,httt,tonggia,donhang,ngaytao,trangthai,pttt,ptgn) 
				  VALUES ('$idmember','$mahoadon','$hoten','$sdtnhanhang','$diachinhanhang','$email','$httt','$tonggia','$body','$ngaydangky',5,'$ptthanhtoan','$ptgiaohang')";	
		 ...
		 foreach ($_SESSION['cart'] as $item_cart) {
            ...
            $item_pro = $d->fetch_array();
				...
				$mail->SetFrom($config_email,$_POST['hoten']);
				...
				*=====================================*/
				...
				//Thiết lập tiêu đề
				$mail->Subject    = $_POST['tieude']." - ".$company['ten_vi'];
				$mail->IsHTML(true);
				...
				$mail->CharSet = "utf-8";	
					...
					if($mail->Send()){
							...
							if(isset($_POST['shipping-save-in-address-book']))
							{
								...
								$d->reset();
								$data = null;
								$data['ten'] = 'Thông tin đặt hàng: Khách hàng <span style="font-weight:bold;">'.$_POST['ten'].'</span> mới gửi 1 mail vào email:'.$company["email"].' của công ty! <a href="index.php?com=donhang&act=man"> Xem </a>';
								$data['ngaytao'] = time();
								$d->setTable('thongbao');
								$d->insert($data);	
					...
					transfer("Đơn hàng của bạn đã được gửi.<br>Cảm ơn.", "san-pham.html");
					}
					else{
					 transfer("Đơn hàng của bạn đã được lưu.<br>Cảm ơn", "san-pham.html");
					}
				 
				 transfer("Đơn hàng của bạn đã được gửi", "san-pham.html");
